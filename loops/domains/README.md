...
**machinery** (`metrics/*.jsonl`, collectors). It **links** to artifacts in `loops/signals/` and
`loops/docs/`; it never contains them. The loop's to-dos live inline in the README's `## Backlog`
(promote to a `task` kind only once that outgrows the README — see `loops/ARCHITECTURE.md`).
...
This README is the schema. See `loops/ARCHITECTURE.md` for the model.
