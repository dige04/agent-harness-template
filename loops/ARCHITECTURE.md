...
|---|---|---|---|
| `signal` | evidence: feedback / idea / observation (deduped, frequency-counted) | `loops/signals/` | `category, frequency, sources[], domain[], status` |
| `doc` | durable knowledge: an analysis, a decision, a thing you learned | `loops/docs/` | `domain[], status?, links` |
...
- **`loops/LOG.md`** (root) — global activity feed: one line per ship/ingest. Detail lives in each
...
- **No separate `daily`/`journal` kind.** A domain's run-log is its `README`'s `## Timeline`
  ...
  exactly two log surfaces: per-artifact `## Timeline` + the global `loops/LOG.md`.
- **`loops/domains/<x>/metrics/*.jsonl`** — numeric time-series, written by **deterministic collectors**
...
| I want to… | Go to |
|---|---|
| record a fact / insight we learned | `loops/docs/` |
| capture feedback / an idea (with frequency) | `loops/signals/` |
| track a piece of committed work | a backlog line in the domain `README` (or `loops/tasks/` once earned) |
| read a deep analysis | `loops/docs/` |
| see why we chose something | `loops/docs/` (a decision) |
| see a loop's goal / cadence / state | `loops/domains/<x>/README.md` |
| see metrics over time | `loops/domains/<x>/metrics/*.jsonl` + scorecard |
